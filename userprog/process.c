...
process_execute (const char *cmdline) 
{
  ...
  if (fn_copy == NULL)
	...
	file_name = strtok_r (fn_copy, " ", &arguments);
  ...
  if (tid == TID_ERROR)
    palloc_free_page (fn_copy); 
	else
		{
			...
			struct child *s = calloc (1, sizeof (struct child));
			if (s)
				{
...
start_process (void *arguments_)
{
  ...
  if (!success) 
		{
			/* If its parent is alive, wake it up. */
			if (cur->parent)
	...
	else
		{
			...
			fdt->fd_map = bitmap_create (OPEN_DEFAULT);	
			if (!fdt->fd_map)
				{
					if (cur->parent)
						parent_wakeup (cur->parent, cur->tid);
			...
			fdt->max_fds = OPEN_DEFAULT;

			files->fdt = fdt;
			files->next_fd = 0;
			lock_init (&files->file_lock);

			int i;
			for (i = 0; i < 32; i++)
				files->fd_array[i] = NULL;

			cur->pid = allocate_pid ();
			cur->self = NULL;
			cur->files = files;
...
load (const char *arguments, void (**eip) (void), void **esp) 
{
  ...
  if (file == NULL) 
	...
	/* Keep it until the process exits. */
	t->exec_file = file;
		   ...
		   && page_add_stack (upage))
				{
					...
					*esp = PHYS_BASE;
					...
					arglen = strlen (arguments) + 1;
					*esp -= arglen;
					memcpy (*esp, arguments, arglen);
					total += arglen;

					arglen = strlen (file_name) + 1;
					*esp -= arglen;
					memcpy (*esp, file_name, arglen);
					total += arglen;
					...
					*esp -= 4 - total % 4;
					...
					/* Push argv[argc] null pointer. */
					*esp -= offset;
					* (uintptr_t *) *esp = (uintptr_t) NULL;

					/* Wind up the command string and replace spacecharacters with terminators.*/
					int i = total - 1;
					while (*(cmdhead + i) == ' ' ||  *(cmdhead + i) == '\0')
						{
							if (*(cmdhead + i) == ' ')
								*(cmdhead + i) = '\0';
							i--;
					...
					/* Search the command string backward by the offset which points to the end of it. */
					char *mark;
					for (mark = (char *)(cmdhead + i); i > 0;
					i--, mark = (char *)(cmdhead + i))
						{
							/* If the pointer gets to the head of a word, push it into the stack and count it as a argument. */
							if ((*mark == '\0' || *mark == ' ') &&
								 (*(mark + 1) != '\0' && *(mark + 1) != ' '))
								{
									*esp -= offset;
									* (uintptr_t *) *esp = (uintptr_t) mark + 1;
									argc++;
								}
					...
					*esp -= offset;
					* (uintptr_t *) *esp = (uintptr_t) cmdhead;
					argc++;

					/* Push argv. */
					* (uintptr_t *) (*esp - offset) = *(uintptr_t *) esp;
					*esp -= offset;
...
/* Returns a pid to use for a new user process. */
static pid_t
allocate_pid (void) 
{
...
void
free_children_list (struct list *list)
{
  struct list_elem *e;
	while (!list_empty (list))
		{
			...
			struct child *s = list_entry (e, struct child, elem);
			/* If its child is alive, make it know that its father is dead. */
			if (s->t)
				{
					s->t->self = NULL;
					s->t->parent = NULL;
...
void
set_return_value (struct child * s, int status)
{
	s->t = NULL;
	s->retval = status;
	s->terminated = true;
...
void
parent_wakeup (struct thread *parent, tid_t tid)
{
	/* If its parent is alive, wake it up. */
	struct list_elem *e;
	for (e = list_begin (&parent->children_list); e != list_end (&parent->children_list);
			 e = list_next (e))
		{
			struct child *s = list_entry (e, struct child, elem);
			if (s->tid == tid)
